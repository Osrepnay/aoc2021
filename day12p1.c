...
void num_paths (struct Cave nodes[], int nodes_len, int *paths, int start, int end, int went_to[], int went_to_len) {
    for (int i = 0; i < nodes[start].connections_len; ++i) {
        ...
        if (islower(nodes[connection].name[0]) && contains(connection, went_to, went_to_len)) {
            continue;
        }
        went_to[went_to_len] = connection; // went_to will contain some big caves, but is_lower above filters it out
        num_paths (nodes, nodes_len, paths, connection, end, went_to, went_to_len + 1);
    }
}
...
int main () {
    ...
    int went_to[50];
    went_to[0] = start;
    num_paths (nodes, nodes_len, &paths, start, end, went_to, 1);
    printf("%d\n", paths);

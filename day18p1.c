...
int main () {
    ...
    while (fscanf(stream, "%s", line) != EOF) {
        PairElem curr_pairs[MAX_PER_LINE];
        int curr_pairs_len = 0;
        ...
        for (int i = 0; line[i] != '\0'; ++i) {
            switch (line[i]) {
                ...
                case ',':
                    on_left = false;
                    curr_pairs[curr_pairs_len++] = (PairElem) { .is_comma = true, .depth = curr_depth };
                    break;
                case ']':
                    --curr_depth;
                    break;
                default:
                    curr_pairs[curr_pairs_len++] = (PairElem) {
                        .is_comma = false,
                        .val = line[i] - '0',
                        .on_left = on_left,
                        .depth = curr_depth
        ...
        bool first_addition = pairs_len == 0;
        for (int i = 0; i < pairs_len; ++i) {
            ++pairs[i].depth;
        }
        if (!first_addition) {
        ...
        for (int i = 0; i < curr_pairs_len; ++i) {
            if (!first_addition) {
                ++curr_pairs[i].depth;
            }
            pairs[pairs_len++] = curr_pairs[i];
        }

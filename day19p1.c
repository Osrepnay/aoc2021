...
continue_search:
    ...
    Point all_beacons[MAX_SCANNERS * MAX_SCANNERS_POINTS];
    int all_beacons_len = 0;
    for (int i = 0; i < scanners_len; ++i) {
        for (int j = 0; j < beacons_lens[i]; ++j) {
            bool found = false;
            for (int b = 0; b < all_beacons_len; ++b) {
                if (points_eq(all_beacons[b], beacons[i][j])) {
                    found = true;
                    break;
                }
            }
            if (!found) {
                all_beacons[all_beacons_len++] = beacons[i][j];
            }
        }
    }
    printf("%d\n", all_beacons_len);
